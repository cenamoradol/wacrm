...
describe('resolveReferencePath', () => {
  ...
  it('resolves a simple vars.* key', () => {
    expect(resolveReferencePath(vars, 'vars.webhook_response')).toBe(
  ...
  it('resolves a nested path', () => {
    expect(resolveReferencePath(vars, 'vars.webhook_response.brands')).toEqual([
      'Honda',
      'Toyota',
    ]);
  });
  ...
  it('resolves an indexed path', () => {
    expect(
      resolveReferencePath(vars, 'vars.webhook_response.results[0].name')
    ).toBe('Labial Matte');
    expect(
      resolveReferencePath(vars, 'vars.webhook_response.results[1].id')
    ).toBe('p2');
  });
  ...
  it('returns undefined for an unresolvable path', () => {
    ...
    expect(
      resolveReferencePath(vars, 'vars.webhook_response.brands[99]')
    ).toBeUndefined();
    expect(
      resolveReferencePath(vars, 'vars.webhook_response.brands.foo')
    ).toBeUndefined();
  });
