...
export function AutomationBuilder({ initial }: { initial: BuilderInitial }) {
  ...
  }, [state.is_active, state.trigger_type, state.trigger_config, state.steps]);
  ...
  return (
    <div className="bg-background fixed inset-0 flex flex-col">
      ...
      <header className="border-border bg-card/80 flex flex-shrink-0 items-center gap-2 border-b px-3 py-3 sm:gap-3 sm:px-4">
        ...
        {activationBlocked && (
          <span className="text-destructive hidden text-xs font-medium sm:inline">
              ...
              <div>
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    onConfigChange({
                      ...config,
                      condition_prompt: e.target.value,
                    })
                  }
...
function StepList(props: StepListProps) {
  const {
    steps,
    parentPath,
    parentScope: parentScopeOverride,
    ...rest
  } = props;
...
function ConditionBranches({
  ...
  const yesScope: ParentScope = {
    kind: 'branch',
    parentCid: step.cid,
    branch: 'yes',
  };
  const noScope: ParentScope = {
    kind: 'branch',
    parentCid: step.cid,
    branch: 'no',
  };
        ...
        <>
          <FieldBlock label={t('config.subjectLabel')}>
            ...
            >
              ...
              <option value="vars_value">
                {t('config.subjects.vars_value')}
              </option>
            </select>
          </FieldBlock>
          ...
          {subject === 'vars_value' && (
            <>
              <FieldBlock
                label={t('config.operatorLabel', { defaultValue: 'Operator' })}
              >
                <select
                  value={operator}
                ...
                >
                  <option value="is_empty">
                    {t('config.operators.is_empty')}
                  </option>
                  <option value="is_not_empty">
                    {t('config.operators.is_not_empty')}
                  </option>
                  <option value="equals">{t('config.operators.equals')}</option>
                  <option value="not_equals">
                    {t('config.operators.not_equals')}
                  </option>
                  <option value="contains">
                    {t('config.operators.contains')}
                  </option>
                </select>
              </FieldBlock>
        ...
        <>
          ...
          <FieldBlock label={t('config.methodLabel')}>
            <select
              value={method}
              onChange={(e) =>
                set({ method: e.target.value as 'GET' | 'POST' })
              }
              className="border-border bg-muted text-foreground w-full rounded-md border px-2 py-1.5 text-sm"
            >
                ...
                {queryParamEntries.map(([k, v], i) => (
                  <div key={`${i}-${k}`} className="flex items-center gap-2">
                    <Input
                      ...
                      className="bg-muted text-foreground flex-1 font-mono text-xs"
                    />
            ...
            <Textarea
              ...
              onChange={(e) => {
                try {
                  ...
                  set({
                    headers: e.target.value as unknown as Record<
                      string,
                      string
                    >,
                  });
              ...
              className="bg-muted text-foreground min-h-16 font-mono text-xs"
            />
          </FieldBlock>
    ...
    case 'extract_vars': {
      ...
      return (
        <>
              ...
              {fieldEntries.map(([name, type]) => (
                <div key={name} className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    className="border-border bg-muted text-foreground w-32 rounded-md border px-2 py-1.5 text-sm"
                  >
          ...
          <FieldBlock
            label={t('config.extractVarsReferencePathLabel', {
              defaultValue: 'Reference vocabulary (optional)',
            })}
          >
            <Input
